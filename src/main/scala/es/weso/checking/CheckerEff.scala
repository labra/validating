...
abstract class CheckerEff {
  ...
  def cond[A,B](c: Check[A], thenPart: A => Check[B], elsePart: Err => Check[B]): Check[B] = for {
    either <- attempt(c)
    b <- either.fold(e => elsePart(e),v => thenPart(v))
  } yield b 
  ...
  def run[A](c: Check[A])(config: Config)(env: Env = Monoid[Env].empty): (Either[Err, A], Log) = {
    ...
    c.runEither.runReader(env).runReader(config).runWriterFold(strFold).run
  }
