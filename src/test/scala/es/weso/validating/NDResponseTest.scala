...
class NDResponseTest extends FunSpec with Matchers with OptionValues {
  ...
  describe("NDResponse") {
    ...
    it("can combine several responses") {
      ...
      val rs1: NDResponse[Int,Option] = NDResponse(Seq(r1,r2))
      val rs2: NDResponse[Int,Option] = NDResponse(Seq(Response(Some(3)),Response(Some(4))))
  ...
  describe("merge") {
   it("can merge responses") {
     val r1: NDResponse[Int,Option] = single(Some(1))
     val r23: NDResponse[Seq[Int],Option] = single(Some(Seq(2,3)))
     val expected: NDResponse[Seq[Int],Option] = single(Some(Seq(1,2,3)))
     r1.merge(r23) should be(expected)
   }
   it("can merge empty response with a list of responses") {
     val r0: NDResponse[Int,Option] = initial
     val r23: NDResponse[Seq[Int],Option] = single(Some(Seq(2,3)))
     val expected: NDResponse[Seq[Int],Option] = single(Some(Seq(2,3)))
